...
注記:
- 本文書が扱う remediation scope (`R0-R5`) は完了済みである。
- 本文書は remediation の履歴記録として維持し、今後の backend expansion roadmap は追加しない。
- 以後の backend 拡張の正本は `docs/progress.md`、`docs/validation-spec.md`、`docs/research-workbench-plan.md` を参照する。
...
| Phase R1: numerical utility の整理 | 完了 | `scipy` 導入、cumulative trapezoid / root finding の共通 utility 化に加え、fixed-point 候補評価 utility と HFB equilibrium の Anderson/DIIS 系 accelerator を実装済み | contour fixed-point への適用は validation row と連動して継続検討 |
...
## 10. 将来の責務分割方針（2026-03-21 コードレビュー由来）

### 背景

コードレビュー（`docs/backend-code-review-2026-03-21.md`）で、以下のモジュールが単一ファイル内に複数責務を混在させている点を指摘された。

| ファイル | 行数 | 混在する責務 |
| --- | --- | --- |
| `self_energy_second_born.py` | 919 | orchestration、数値カーネル、contour branch ルーティング、diagnostics 組み立て |
| `self_energy_second_born_prototype.py` | 779 | 同上（prototype 版） |
| `kbe_hfb.py` | 599 | 外部 solve API、trajectory 解析、diagnostics 集約 |

### 推奨分割単位

**`self_energy_second_born.py` の分割候補:**

- `_kernel.py`: 数値カーネル（`_build_gkba_row_data`, `_build_local_second_born_self_energy`, `_damping_collision`, `_stabilized_kernel` 等）
- `_contour.py`: contour branch 別ルーティング（thermal branch, mixed branch, keldysh 分岐ロジック）
- `_diagnostics.py`: diagnostics 組み立て・検証（`_base_second_born_diagnostics`, diagnostics update ブロック）

**`kbe_hfb.py` の分割候補:**

- `_orchestration.py`: `solve` エントリポイントと branch 呼び出し順序
- `_trajectory.py`: trajectory 解析・observable 計算（`_analyze_trajectory` 等）

### 実装方針

- 実装タイミング: 機能追加・バグ修正ではなく、独立した refactor タスクとして扱う。
- 分割前に unit テスト粒度を上げてから実施すると回帰検知コストが下がる（`test_self_energy_second_born.py` の拡充が前提）。
- public interface（`compute_second_born_reference_correction`, `compute_second_born_prototype_correction`）の署名は変えない。分割は内部モジュールとして `_` prefix で整理する。

...
- compare / sweep の backend 足場として、`job group` / `sweep` / `derived analysis artifact` の registry table / schema / API を追加し、run-child lineage と parent artifact state aggregation を registry DB で保持できるようにした。さらに `job-groups/launch` / `sweeps/launch` により、base config から child run を生成して parent artifact と run metadata を同期する actual launch semantics を backend に追加した。
...
| 項目 | 状態 | 現状 | 次に必要なこと |
| --- | --- | --- | --- |
| experiment registry | 部分完了 | SQLite-backed な `experiment registry`、`ExperimentRepository`、DB-backed `/runs` list、`study` / `decision note` / `evidence bundle` API、run metadata patch、backfill utility に加えて、`job group` / `sweep` / `derived analysis artifact` の registry table / API / relation schema を追加し、run-child lineage と parent artifact state aggregation を DB 側で保持できる。`job group` / `sweep` については base config から child run を起動する launch semantics と、`group_id` / `sweep_id` / `variant_label` の run metadata 同期まで backend で閉じた | analysis の再計算・result payload・再取得面を backend artifact lifecycle へ広げる |
| API / schema | 部分完了 | `/health`, `/schema/simulation`, `/presets`, `/runs`, `/runs/{id}`, `/runs/{id}/metadata`, `/runs/{id}/log`, `/runs/{id}/observables`, `/runs/{id}/green-functions`, `/runs/{id}/thermal-branch`, `/runs/{id}/mixed-green-functions`, `/studies`, `/job-groups`, `/job-groups/launch`, `/sweeps`, `/sweeps/launch`, `/decision-notes`, `/derived-analyses`, `/evidence-bundles` が実装済みで、`kbe` / `adaptive` / `thermal_branch` 設定と research artifact schema を API から取得できる。`/presets` には provisional な `square-4x4-higgs-demo-kbe-hfb` を追加し、`kbe_hfb + hfb + bond_d` の long-window Gaussian pulse demo draft を API から取得できる | analysis payload 再取得面、enriched `presets`、study-aware URL state を追加する |
...
### 3.1 frontend は P1 shell に着手し、backend は compare / sweep の launch semantics を持ったが actual surface 接続は未完
...
- backend には `job group` / `sweep` / `derived analysis artifact` API が入り、child run lineage と parent state aggregation を保持できるようになった。さらに `job-groups/launch` / `sweeps/launch` により、base config から child run を生成して `group_id` / `sweep_id` / `variant_label` を同期する launch semantics を持つ。
- ただし `Compare Jobs` / `Parameter Sweep` は、これらの backend resource をまだ fetch / re-read / result payload に接続していないため、現時点では planning placeholder に留まる。
...
- solver unit test には厳密極限、構造保存、2x2 exact diagonalization による短時間 / longer-window benchmark、非相互作用および Phase 2-3 weak-coupling row の `\Delta t` 収束回帰、Phase 1-3 の continuity residual regression、thermal branch の exact density/current 比較、adaptive tolerance の fine fixed-step 参照比較、および memory window dependence row を追加した。一方で、長時間安定性、系サイズ依存、paired interacting benchmark、独立 benchmark との比較は未整備である。
...
- compare / sweep launch semantics を追加
  - `backend/app/schemas/research.py` に `JobGroupLaunchRequest` / `SweepLaunchRequest` を追加し、launch と metadata-only create を分離した
  - `/job-groups/launch` と `/sweeps/launch` を追加し、base config から child run を生成したうえで parent artifact を作成できるようにした
  - `ExperimentRepository` で `group_id` / `sweep_id` に加えて `variant_label` を child run metadata へ同期するようにした
  - workflow test として launch 後の child run config / metadata 同期、および invalid `parameter_path` の 422 回帰を追加した
...
少なくとも現時点で、`job group` / `sweep` / `derived analysis artifact` の backend metadata / lineage / state aggregation は workflow regression と backend 全体 test で破綻しておらず、`job-groups/launch` / `sweeps/launch` による actual child-run launch も backend では成立している。frontend 既定 outDir build は root-owned 生成物に阻まれたが、別 outDir build では API 追加後も bundling 自体は成立している。
...
- `job group` / `sweep` / `derived analysis artifact` の result payload / 再取得面を backend artifact lifecycle に広げる
